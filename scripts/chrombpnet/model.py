...
class ChromBPNet(nn.Module):
    ...
    def __init__(self, filters=8, conv_kernel_size=21, profile_kernel_size=7, num_layers=6, seqlen=300, outlen=150):
    ...
    def forward(self, x):  # Input size (batch, 4, seqlen)
        ...
        crop_size = (self.outlen - prof_precrop.shape[2])//2
        assert crop_size>=0
