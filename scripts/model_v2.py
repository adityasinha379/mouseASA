...
class quaidScan(nn.Module):
    ...
    def forward(self, x): # (batch, 2, 4, 300)
        ...
        x = self.seq_extractor_2(x)
        x = torch.flatten(x,1)
        h_diff = x.clone().view(2,-1,x.shape[-1])   # (2, batch, d)
        h_sum = x.view(2,-1,x.shape[-1])
        y_sum = self.sumhead(h_sum[0],h_sum[1]) 
        y_fc = self.fchead(h_diff[0],h_diff[1])
        return y_sum, y_fc